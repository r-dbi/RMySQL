...
int
RS_DBI_getDriverNameIndex(const char *drvName)
{
  int i;
  for(i=0; ; i++){
    if(strcmp(drvName, RS_DBI_driverTable[i].drvName)==0)
      return i;
  }
  return i;
}
...
int
RS_DBI_getDriverIndex(Sint drvId)
{
  int indx;
  for(indx=0; ;indx++){
    if(!RS_DBI_driverTable[indx].drvId)
      break;
    if(RS_DBI_driverTable[indx].drvId == drvId)
      return indx;
  }
  return indx;
}

#define RS_DBI_getDriverName(indx) RS_DBI_driverTable[indx].drvName
#define RS_DBI_getDriverId(indx) RS_DBI_driverTable[indx].drvId
#define RS_DBI_getDriverMgr(indx) RS_DBI_driverTable[indx].mgr
#define RS_DBI_setDriverMgr(indx, mgr) RS_DBI_driverTable[indx].mgr = mgr

RS_DBI_manager *
RS_DBI_getDriver(Sint mgrId)
{
  Sint indx = RS_DBI_getDriverIndex(mgrId);
  return RS_DBI_getDriverMgr(indx);
}
...
{
  /* Currently, drivers are singleton (therefore we don't 
  ...
  RS_DBI_manager *mgr;
  Sint counter;
  Sint pid  = (Sint) getpid();
  int  indx;
  Sint mgr_id;
  int i;

  indx = RS_DBI_getDriverNameIndex(drvName);
  mgr_id = RS_DBI_getDriverId(indx);
  mgrHandle = RS_DBI_asMgrHandle(pid, mgr_id);
  mgr = RS_DBI_getDriverMgr(indx);
  if(!mgr){                            /* alloc for the first time */
    counter = 0;                       /* connections handled so far */
    mgr = (RS_DBI_manager*) malloc(sizeof(RS_DBI_manager));
    RS_DBI_setDriverMgr(indx, mgr);
  }
  else {                               /* we're re-entering */
    if(mgr->connections){              /* and mgr is valid */
      if(!force_realloc)            
	return mgrHandle;
      else
...
/* We don't want to completely free the driver, but rather we 
...
RS_DBI_allocConnection(Mgr_Handle *mgrHandle, Sint max_res)
{
  ...
  conHandle = RS_DBI_asConHandle(PID(mgrHandle), MGR_ID(mgrHandle), con_id);
  return conHandle;
}
...
RS_DBI_freeConnection(Con_Handle *conHandle)
{
  ...
  if(con->num_res > 0) {
    ...
    for(i=0; i < con->num_res; i++){
      rsHandle = RS_DBI_asResHandle(con->processId,
                                    con->managerId,
				    con->connectionId,
  ...
  indx = RS_DBI_lookupEntry(mgr->connectionIds, mgr->length, con->connectionId);
  RS_DBI_freeEntry(mgr->connectionIds, indx);
...
RS_DBI_allocResultSet(Con_Handle *conHandle)
{
  ...
  result->processId = PID(conHandle);
  ...
  rsHandle = RS_DBI_asResHandle(PID(conHandle), 
                                MGR_ID(conHandle),
                                CON_ID(conHandle),
                                res_id);
  return rsHandle;
...
RS_DBI_freeResultSet(Res_Handle *rsHandle)
{
  ...
  /* update connection's resultSet table */
  indx = RS_DBI_lookupEntry(con->resultSetIds, con->length, RES_ID(rsHandle));
  RS_DBI_freeEntry(con->resultSetIds, indx);
...
{ 
   ...
   switch( (int) GET_HANDLE_TYPE(handle)){
     case MGR_HANDLE_TYPE: handleType = MGR_HANDLE_TYPE; break;
     case CON_HANDLE_TYPE: handleType = CON_HANDLE_TYPE; break;
     case RES_HANDLE_TYPE: handleType = RES_HANDLE_TYPE; break;
   }
		    ...
		    int errorNum, const char *errorMsg)
{
  ...
  handleType = (int) GET_HANDLE_TYPE(handle);
...
Mgr_Handle *
RS_DBI_asMgrHandle(Sint pid, Sint mgrId)
{
  Mgr_Handle *mgrHandle;

  MEM_PROTECT(mgrHandle = NEW_INTEGER((Sint) 2));
  PID(mgrHandle) = pid;
...
Con_Handle *
RS_DBI_asConHandle(Sint pid, Sint mgrId, Sint conId)
{
  Con_Handle *conHandle;

  MEM_PROTECT(conHandle = NEW_INTEGER((Sint) 3));
  PID(conHandle) = pid;
...
Res_Handle *
RS_DBI_asResHandle(Sint pid, Sint mgrId, Sint conId, Sint resId)
{
  Res_Handle *resHandle;

  MEM_PROTECT(resHandle = NEW_INTEGER((Sint) 4));
  PID(resHandle) = pid;
...
RS_DBI_getManager(Mgr_Handle *handle)
{
  ...
  if(!is_validHandle(handle, MGR_HANDLE_TYPE))
    RS_DBI_errorMessage("invalid manager handle", RS_DBI_ERROR);
  mgr = RS_DBI_getDriver(MGR_ID(handle));
  if(!mgr)
    RS_DBI_errorMessage(
          "internal error in RS_DBI_getManager: corrupt manager handle",
	  RS_DBI_ERROR);
  return mgr;
}
...
RS_DBI_getConnection(Con_Handle *conHandle)
{
  ...
  mgr = RS_DBI_getManager(conHandle);
  indx = RS_DBI_lookupEntry(mgr->connectionIds, mgr->length, CON_ID(conHandle));
  if(indx < 0)
    RS_DBI_errorMessage(
...
/* create an object of class dbHandle (simply an external pointer 
 * with the appropriate class (dbDriver, ...) that extend dbHandle
 */
Db_Handle
RS_DBI_makeDbHandle((void *) ptr, handleClass)
{
  s_object  clsObj, ref;
  Db_Handle handle;

  PROTECT(clsObj = MAKE_CLASS(handleClass));
  if(clsObj==NULL || clsObj==R_NilValue){
    char msg[128];
    (void) sprintf(msg, "could not instantiate class %s", handleClass);
    R_DBI_errorMessage(msg, RS_DBI_ERROR);
  }
  PROTECT(handle = NEW(clsObj));
  PROTECT(ref = R_MakeExternalPtr(ptr, Rf_install(handleClass), R_NilValue));
  R_RegisterCFinalizer(ref, RS_DBI_dbHandle_finalizer);
  SET_SLOT(handle, Rf_install("ref"), ref);
...
void
RS_DBI_dbObject_finalizer(SEXP handle)
{
  if(IS(handle, MAKE_CLASS("DBIDriver")))
    RS_DBI_freeManager(handle);
  else if(IS(handle, MAKE_CLASS("DBIConnection")))
    RS_DBI_freeConnection(handle);
  else if(IS(handle, MAKE_CLASS("DBIResult")))
    RS_DBI_freeResultSet(handle);
...
Sint
RS_DBI_lookupEntry(Sint *table, Sint length, Sint obj_id)
{
  Sint i, indx;
...
/* produce a list of entries into *entries whose space is allocated
 * and freed by the caller.  The function returns the number of entries.
 */
Sint 
...
is_validHandle(Db_Handle *handle, HANDLE_TYPE handleType)
{
  ...
  htype = GET_HANDLE_TYPE(handle);
  if(htype<handleType || handleType<1 || handleType>3) 
    return 0;
  if( ((Sint) getpid()) != PID(handle))
    return 0;

  /* at least we have a potential valid manager  */
  mgr_id = MGR_ID(handle);
  mgr = RS_DBI_getDriver(mgr_id);
  if(!mgr || !mgr->connections)  return 0;   /* expired manager*/
  if(handleType == MGR_HANDLE_TYPE) return 1;     /* valid manager id */

  /* ... on to connections */
  indx = RS_DBI_lookupEntry(mgr->connectionIds, mgr->length, CON_ID(handle));
  if(indx<0) return 0;
  ...
  /* .. on to resultSets */
  indx = RS_DBI_lookupEntry(con->resultSetIds, con->length, RES_ID(handle));
  if(indx < 0) return 0;
...
RS_DBI_managerInfo(Mgr_Handle *mgrHandle)
{
  ...
  char *mgrDesc[] = {"connectionIds", "fetch_default_rec",
                     "processId", "managerId", "length", "num_con", 
                     "counter", "clientVersion"};
  Stype mgrType[] = {INTEGER_TYPE, INTEGER_TYPE, INTEGER_TYPE, INTEGER_TYPE,
		     INTEGER_TYPE, INTEGER_TYPE, INTEGER_TYPE, 
                     CHARACTER_TYPE};
  ...
  LST_INT_EL(output,2,0) = (Sint) mgr->processId;
  ...
  SET_LST_CHR_EL(output,7,0,C_S_CPY("NA"));   /* client versions? */
